...
extern const uint8_t _binary_______build_kernel_hankaku_bin_start;  // ディレクトリパスの影響を受けてる・・・(T_T)回避したい
extern const uint8_t _binary_______build_kernel_hankaku_bin_end;
extern const uint8_t _binary_______build_kernel_hankaku_bin_size;

namespace {

    const uint8_t* GetFont(char c) {
        auto index = 16 * static_cast<unsigned int>(c);
        if (index >= reinterpret_cast<uintptr_t>(&_binary_______build_kernel_hankaku_bin_size)) {
            return nullptr;
        }
        return &_binary_______build_kernel_hankaku_bin_start + index;
    }
...
void InitializeFont() {
    ...
    if (entry == nullptr || pos_slash) {
        Log(kError, "nihongo.ttf not found\n");
        exit(1);
    }
