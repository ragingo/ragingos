    ...
    while ((c = getc(fp)) != EOF) {
        ...
        if (c < 0x80) {
            ...
            if (c2 != EOF && (c2 & 0xc0) == 0x80) {
                ...
                if (c2 != EOF)
                    current_line.push_back(INVALID_BYTE | c2);
            }
        } else if ((c & 0xf0) == 0xe0) {
            int32_t c2 = getc(fp);
            int32_t c3 = getc(fp);
            if (c2 != EOF && (c2 & 0xc0) == 0x80 &&
                ...
                if (c2 != EOF)
                    current_line.push_back(INVALID_BYTE | c2);
                if (c3 != EOF)
                    current_line.push_back(INVALID_BYTE | c3);
            }
        } else if ((c & 0xf8) == 0xf0) {
            int32_t c2 = getc(fp);
            int32_t c3 = getc(fp);
            ...
            if (c2 != EOF && (c2 & 0xc0) == 0x80 &&
                ...
                if (c3 != EOF)
                    current_line.push_back(INVALID_BYTE | c3);
                if (c4 != EOF)
                    current_line.push_back(INVALID_BYTE | c4);
            }
        } else {
            current_line.push_back(INVALID_BYTE | c);
        }
            ...
            if (arg.keypush.press) {
                ...
                if (arg.keypush.modifier == KEY_MODIFIER_CTRL &&
                    ...
                    if (arg.keypush.ascii == '\b') {
                        ...
                        // Enter
                        int idx = cursor_x == 0 ? 0 : char_idx[cursor_y - scroll_y][cursor_x - 1] + 1;
